...
### :grey_question:3. (0.75 points) Provided that there is a 10 MHz bandwidth channel, we transmit a 32-QAM signal.
...
### :grey_question:4. (0.5 points) Provided that there is a UTP-5 cable, that the attenuation is -220 dB/km for a 100 MHz frequency, that we transmit with 2W power and we need to receive 2 mW at least, calculate the maximum cable length.
...
### :grey_question:6. (0.5 points) We want to interconnect two Ethernet 100BaseTX hubs. Considering that to every hub the stations can be connected through cables whose maximum length is 50m, that the maximum hubs separation is 100m, and that the network introduces the delays hereafter described, determine what is the maximum Fast Ethernet hub delay to implement a feasible network.
...
### :grey_question:13. (0.5 points) Draw a scheme showing the TCP segments which are interchanged between the client and the server in the following scenarios:
